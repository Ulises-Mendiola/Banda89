import { socialLinks } from '../data'

// SVG personalizado de Facebook con estética de la banda
const FacebookIcon = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="26" height="26" aria-hidden="true">
        <path d="M24 12.073C24 5.404 18.627 0 12 0S0 5.404 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.792-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.97h-1.513c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" />
    </svg>
)

// SVG personalizado de Instagram
const InstagramIcon = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="26" height="26" aria-hidden="true">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
)

export default function Footer() {
    return (
        <footer className="footer-new" role="contentinfo" aria-label="Pie de página de Banda 89 El Reencuentro Oficial">

            {/* ── Decoración superior ─────────── */}
            <div className="footer-top-deco" aria-hidden="true">
                <div className="footer-deco-line" />
                <span className="footer-deco-diamond">◆</span>
                <div className="footer-deco-line" />
            </div>

            {/* ── Logo central ─────────────────── */}
            <div className="footer-logo-block">
                <div className="footer-logo-text">BANDA 89</div>
                <div className="footer-logo-sub">El Reencuentro Oficial</div>
            </div>

            {/* ── CTA + Redes sociales ─────────── */}
            <div className="footer-cta-block">
                <p className="footer-cta-text">
                    ¡Únete a nuestra familia! Síguenos en redes y no te<br />
                    pierdas ningún concierto, novedad ni sorpresa.
                </p>

                <div className="footer-social-row" role="list">

                    {/* Facebook */}
                    <a
                        id="footer-facebook-btn"
                        href={socialLinks.facebook}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="footer-social-card fb-card"
                        role="listitem"
                        aria-label="Seguir Banda 89 en Facebook"
                    >
                        <div className="social-icon-wrap">
                            <FacebookIcon />
                        </div>
                        <div className="social-card-info">
                            <span className="social-card-network">Facebook</span>
                            <span className="social-card-action">Seguir página →</span>
                        </div>
                    </a>

                    {/* Instagram */}
                    <a
                        id="footer-instagram-btn"
                        href={socialLinks.instagram}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="footer-social-card ig-card"
                        role="listitem"
                        aria-label="Seguir Banda 89 en Instagram"
                    >
                        <div className="social-icon-wrap">
                            <InstagramIcon />
                        </div>
                        <div className="social-card-info">
                            <span className="social-card-network">Instagram</span>
                            <span className="social-card-action">Seguir perfil →</span>
                        </div>
                    </a>

                </div>
            </div>

            {/* ── Copyright ─────────────────────── */}
            <div className="footer-copy">
                <div className="footer-copy-divider" aria-hidden="true" />
                <p>© {new Date().getFullYear()} <strong>Banda 89 El Reencuentro Oficial</strong>. Todos los derechos reservados.</p>
                <p className="footer-copy-sub">Hecho con <span className="footer-heart">♥</span> en Mazatlán, Sinaloa, México</p>
                <div className="footer-dev-signature">
                    Diseñado y Desarrollado por <a href="https://wa.me/525662971609?text=Hola%20que%20tal%2C%20Me%20encant%C3%B3%20la%20experiencia%20visual%20de%20la%20web%20de%20Banda%2089.%20Me%20gustar%C3%ADa%20saber%20m%C3%A1s%20sobre%20tus%20servicios%20de%20desarrollo." target="_blank" rel="noopener noreferrer">Click & Go</a>
                </div>
            </div>

        </footer>
    )
}
